  ...
  global.servedContractsIds.forEach(async (contractId) => {
    ...
    for (let interaction of contractInteractions) {
      if (!databases.interactions[contractId]) {
        databases.interactions[contractId] = databases.interactions[
          contractId
        ] = lmdb.open("./db/interactions/" + contractId);
      }
      console.log(interaction);
      interaction = await databases.interactions[contractId].get(interaction);
      interactionIndex++;
      let alreadyIndexed = await databases.evaluationResults.get(
        contractId + interactionIndex
      );
      ...
      if (interactionIndex == 0) {
        //We're syncing up from ground, need to download init state and init code

        try {
          contractSrc = contractInstantiateTx.tags.find(
            (tag) => tag.name == "Contract-Src"
          )?.value;
          state = contractInstantiateTx.tags.find(
            (tag) => tag.name == "Init-State"
          )
            ? JSON.parse(
                contractInstantiateTx.tags.find(
                  (tag) => tag.name == "Init-State"
                ).value
              )
            : JSON.parse(await fetchTxContent(contractId));
        } catch (e) {
          return null;
        }
      } else {
        state = (
          await databases.evaluationResults.get(
            contractId + (interactionIndex - 1)
          )
        ).state;
        contractSrc = state.evolve
          ? state.evolve
          : (
              await databases.evaluationResults.get(
                contractId + (interactionIndex - 1)
      ...
      let newState;
      try {
        newState = await execute(
          contractSrc,
          state,
          interaction,
          contractInstantiateTx
        );
      } catch (e) {
    ...
    if (amountOfInteractions === 0) {
      ...
      try {
        ...
        state = contractInstantiateTx.tags.find(
          ...
          ? JSON.parse(
              contractInstantiateTx.tags.find((tag) => tag.name == "Init-State")
                .value
            )
          : JSON.parse(await fetchTxContent(contractId));
      } catch (e) {
